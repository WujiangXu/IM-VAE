...
class IMVAE(nn.Module):
    ...
    def __init__(self, item_length, item_emb_dim, hid_dim, p_dims, q_dims=None, trans_encoder=None, cs_setting=None, dropout=0.5):
        ...
        #     nn.Linear(item_emb_dim*2, item_emb_dim),
        #     nn.ReLU(),
        #     nn.Linear(item_emb_dim, 100),
        #     nn.ReLU(),
        #     nn.Linear(100, 1),
        # )
        # self.discri_criterion = nn.BCEWithLogitsLoss()

        # device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        # self.prior_mu = torch.rand(1).expand_as(torch.zeros(self.q_dims[-1])).to(device)
        # self.prior_logvar = torch.rand(1).expand_as(torch.ones(self.q_dims[-1])).to(device)
        # self.prior_mu2 = torch.rand(1).expand_as(torch.zeros(self.q_dims[-1])).to(device)
        # self.prior_logvar2 = torch.rand(1).expand_as(torch.ones(self.q_dims[-1])).to(device)
    ...
    def forward(self,u_node,i_node,neg_samples,seq_d1,seq_d2,corr_seq_d1,corr_seq_d2,cs_label,domain_id,isTrain):
        ...
        if self.cs_setting:
            KLD1_t =  self._kld_gauss(mu2_t, logvar2_t, mu2_r, logvar2_r)  #-0.5 * torch.sum(-logvar2_t + logvar2 - 1  + torch.exp(logvar2_t - logvar2) + (mu2_t - mu2).pow(2) / logvar2.exp()) 
            KLD2_t =  self._kld_gauss(mu_t, logvar_t, mu_r, logvar_r) #- 0.5 *  torch.sum(-logvar_t + logvar - 1  + torch.exp(logvar_t - logvar) + (mu_t - mu).pow(2) / logvar.exp())
        else:
            KLD1_t =  self._kld_gauss(mu2_t, logvar2_t, mu2, logvar2)  #-0.5 * torch.sum(-logvar2_t + logvar2 - 1  + torch.exp(logvar2_t - logvar2) + (mu2_t - mu2).pow(2) / logvar2.exp()) 
            KLD2_t =  self._kld_gauss(mu_t, logvar_t, mu, logvar) #- 0.5 *  torch.sum(-logvar_t + logvar - 1  + torch.exp(logvar_t - logvar) + (mu_t - mu).pow(2) / logvar.exp())

        KLD1_aug =  self._kld_gauss(mu_aug, logvar_aug, mu, logvar) # -0.5 * torch.sum(-logvar_aug + logvar - 1  + torch.exp(logvar_aug - logvar) + (mu_aug - mu).pow(2) / logvar.exp()) 
        KLD2_aug =  self._kld_gauss(mu2_aug, logvar2_aug, mu2, logvar2) #-0.5 * torch.sum(-logvar2_aug + logvar2 - 1  + torch.exp(logvar2_aug - logvar2) + (mu2_aug - mu2).pow(2) / logvar2.exp()) 
    ...
    #         sigma_1 = 0.1 + 0.9 * F.softplus(torch.exp(logsigma_1)) #exp(0) = 1
    ...
    # def contrastive_loss(self,z,z_aug):
    #     batch_size = z.size(0)
    #     emb_dim = z.size(1)
    #     device = z.device
    #     positive_pairs = torch.diag_embed(torch.ones(batch_size, device=device)) #[bs, bs]
    #     z_cat = torch.cat((z,z_aug), dim = 1)
    #     positive_scores = self.discri(z_cat).squeeze()  
    #     positive_scores = torch.diag(positive_scores)    #bs x bs
    #     positive_scores *= positive_pairs


    #     negative_pairs = 1 - positive_pairs
    #     z_expanded = z.unsqueeze(1).repeat(1, batch_size, 1)  # (batch_size, batch_size, emb_dim)
    #     z_aug_expanded = z_aug.unsqueeze(0).repeat(batch_size, 1, 1)  # (batch_size, batch_size, emb_dim)
    #     z_expanded_cat = torch.cat((z_expanded.view(-1, emb_dim), z_aug_expanded.view(-1, emb_dim)), dim = 1)
    #     negative_scores = self.discri(z_expanded_cat)
    #     negative_scores = negative_scores.view(batch_size, batch_size)
    #     negative_scores *= negative_pairs
    #     labels = torch.diag_embed(torch.ones(batch_size, device=device))
    #     scores = positive_scores + negative_scores
    #     scores = scores.view(-1)
    #     labels = labels.view(-1)
    #     contra_loss = self.discri_criterion(scores, labels)
    #     return contra_loss


if __name__ == '__main__':
    u_node = torch.LongTensor(torch.ones(64).long()).cuda()
    i_node = torch.LongTensor(torch.ones(64).long()).cuda()
    seq_d1 = torch.LongTensor(torch.ones(64,25).long()).cuda()
    seq_d2 = torch.LongTensor(torch.ones(64,25).long()).cuda()
    corr_seq_d1 = torch.LongTensor(torch.ones(64,50).long()).cuda()
    corr_seq_d2 = torch.LongTensor(torch.ones(64,50).long()).cuda()
    all_seq = torch.LongTensor(torch.ones(64,50).long()).cuda()
    long_tail_mask_d1 = torch.LongTensor(torch.ones(64).long()).cuda()
    long_tail_mask_d2 = torch.LongTensor(torch.ones(64).long()).cuda()
    domain_id = torch.LongTensor(torch.ones(64).long()).cuda()
    neg_samples = torch.LongTensor(torch.ones(64,99).long()).cuda()

    u_feat = torch.LongTensor(torch.ones(64,128).long()).float().cuda()
    # innermatch = InnerGraphMatchingLayer(128).cuda()
    # output = innermatch(u_feat,long_tail_mask_d1)
    #print(output.shape) #[bs,emb]
    model = C2DSR(123126, 128, 312321, 128, 25, 32).cuda()
    logits_d1, logits_d2, logits_d1_cross, logits_d2_cross, logits_true_d1, logits_true_d2, logits_false_d1, logits_false_d2 = model(u_node,i_node,neg_samples,seq_d1,seq_d2,corr_seq_d1,corr_seq_d2,all_seq)
    print(logits_d1.shape,logits_d2.shape,logits_d1_cross.shape,logits_d2_cross.shape,logits_true_d1.shape,logits_true_d2.shape,logits_false_d1.shape,logits_false_d2.shape)
