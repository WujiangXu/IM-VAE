...
def train(model,trainLoader,args_config,valLoader):
    ...
    for epoch in range(args_config.epoch):
        ...
        for i,sample in enumerate(tqdm(trainLoader)):
            ...
            loss_cls = torch.mean(loss_cls)
            aug_weight = torch.exp(0.8 * len_d1 / seq_d1.size()[1]) - 0.8
            aug_weight2 = torch.exp(0.8 * len_d2 / seq_d2.size()[1]) - 0.8 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ol_ratio', type=float, default=1.0, help='Lambda for KLD in domain2')  
    parser.add_argument('--ds_ratio', type=float, default=1.0, help='Lambda for KLD in domain2')   
