...
def load_Recmodel(csv_path, ol_ratio):
...
def load_hash_table(args,csv_path, ol_ratio):
    if "cloth" in csv_path:
        domain = 'cloth_sport'
    elif "phone" in csv_path:
        domain = 'phone_elec'
...
def generate_corr_seq(args,csv_path, ol_ratio = 100, batch_size = 1024):
    df = pd.read_csv(csv_path)
    Recmodel, n_users = load_Recmodel(csv_path, ol_ratio)
    ...
    hash_table_d2 = load_hash_table(args, csv_path, ol_ratio)
    ...
    for batch_user in tqdm(user_batches_gpu , total = len(user_batches_gpu )):
        ratings = Recmodel.getUsersRating(torch.Tensor(batch_user).long().to(device))
        ...
        for idx,user_id in enumerate(batch_user ):
            ...
            seq2_items = [item for item in seq2_items if item < ratings_d2.size()[1]]
    ...
    if 'test' in base_path:
        df.to_csv(base_path + str(ol_ratio) + '_updated' + extension)
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch_size', type=int, default=1024, help='the length of the sequence') # 20 for mybank 150 for amazon
    parser.add_argument('--dataset', type=str, default='game_video', help='choose from [game_video, cloth-sport, phone-elec]') # 20 for mybank 150 for amazon


    args = parser.parse_args()

    domain_name = args.dataset
    csv_paths = ['dataset/{}_train.csv'.format(domain_name)] 
    csv_paths += ['dataset/{}_test_csu.csv'.format(domain_name)] 
    for csv_path in csv_paths:
        generate_corr_seq(args, csv_path)
